...
class Agent:
  ...
  def __init__(self,param_set):
    ...
    with open("parameters.yaml","r") as f :
      all_param_set = yaml.safe_load(f)                          # covert yaml into python dict.
  ...
  def run(self,is_training = True,render = False):
    ...
    for episode in itertools.count():
      ...
      while (not terminated and episode_reward < self.reward_threshold):
        ...
        if is_training:
          memory.append((state,action,next_state,reward,terminated))     # 1 experience
          steps += 1

        state = next_state

      print(f"for episode = {episode+1} with total rewards = {episode_reward} and epsilon = {epsilon}")
    ...
    # env.close() - as we will manually stop
